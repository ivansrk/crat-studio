...
export async function ensureProgress(userId: string, courseSlug: string, lessonId: string) {
  if (!getLesson(courseSlug, lessonId)) throw new Error(`unknown lesson: ${lessonId}`)
 ...
 *  MC-05: where обязательно с courseSlug — иначе max(attempt) читался бы через все курсы студента
 *  (латентный баг, найден спек-ревью, тот же что и в @@unique QuizResult). */
export async function startAttempt(userId: string, courseSlug: string, lessonId: string): Promise<QuizResult> {
  await ensureProgress(userId, courseSlug, lessonId)
  for (let tryN = 0; tryN < 2; tryN++) {
    const last = await db.quizResult.findFirst({ where: { userId, courseSlug, lessonId }, orderBy: { attempt: 'desc' } })
    try {
      return await db.quizResult.create({
        data: { userId, courseSlug, lessonId, attempt: (last?.attempt ?? 0) + 1, answers: [] },
      })
    } catch (e) {
...
export async function recordAnswer(userId: string, courseSlug: string, lessonId: string, attemptId: string, questionIndex: number, chosen: number): Promise<AnswerOutcome> {
  const attempt = await db.quizResult.findFirst({ where: { id: attemptId, userId, courseSlug, lessonId } })
  ...
  if (finished && passed) await onQuizPassed(userId, courseSlug, lessonId) // LES-09: зачтённый остаётся зачтённым
...
async function onQuizPassed(userId: string, courseSlug: string, lessonId: string) {
  await db.lessonProgress.updateMany({
    where: { userId, courseSlug, lessonId, quizPassedAt: null },
    data: { quizPassedAt: new Date() },
  })
  await recomputeCompletion(userId, courseSlug, lessonId)
}

/** LES-11: чекбокс практики; снятие → practiceDoneAt = null (completedAt НЕ откатывается — E16). */
export async function setPractice(userId: string, courseSlug: string, lessonId: string, done: boolean) {
  await ensureProgress(userId, courseSlug, lessonId)
  await db.lessonProgress.update({
    where: { userId_courseSlug_lessonId: { userId, courseSlug, lessonId } },
    data: { practiceDoneAt: done ? new Date() : null },
  })
  if (done) await recomputeCompletion(userId, courseSlug, lessonId)
}
...
export async function recomputeCompletion(userId: string, courseSlug: string, lessonId: string) {
  const p = await db.lessonProgress.findUnique({ where: { userId_courseSlug_lessonId: { userId, courseSlug, lessonId } } })
  ...
  await db.deferredQuizState.upsert({ // LES-13
    where: { userId_courseSlug_lessonId: { userId, courseSlug, lessonId } },
    update: {},
    create: { userId, courseSlug, lessonId, dueAt: new Date(completedAt.getTime() + DEFERRED_DAYS_MS) },
  })
  ...
  await checkAndIssueCertificate(userId, courseSlug).catch(e => console.error('[cert] выдача после урока:', e))
}
...
export async function getLessonState(userId: string, courseSlug: string, lessonId: string): Promise<LessonState> {
  const p = await db.lessonProgress.findUnique({ where: { userId_courseSlug_lessonId: { userId, courseSlug, lessonId } } })
  return {
...
export async function getCourseProgress(userId: string, courseSlug: string) {
  const rows = await db.lessonProgress.findMany({ where: { userId, courseSlug } })
